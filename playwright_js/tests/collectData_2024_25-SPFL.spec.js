...
async function clickStats(page) {
  ...
  await page
    .click('button:has-text("Stats")', { timeout: 4000 })
    .catch(async () => {
      // simple retry if first click happens too early
      await page.waitForTimeout(4000);
      await page.click('button:has-text("Stats")', { timeout: 4000 });
    });
}
...
async function getStatPair(page, label, numericParser) {
  ...
  for (const roundKey of roundKeys) {
    ...
    const fixtureList = fixtures[roundKey] || [];
    if (roundIndex % 2 === 0) {
      await page.waitForTimeout(2000);
    }

    await page.goto(
      `https://www.fotmob.com/en-GB/leagues/64/fixtures/premiership?season=2024-2025&group=by-round&round=${roundIndex}`
    );
    ...
    saveJSON("./collectedData/SPFL_data_2024_25.json", seasonData);
  }
});
